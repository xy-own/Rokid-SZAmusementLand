Shader "<effect>_ML_uv/ParticleFx_Pa_Add_ND_UV2" {
	Properties{
		...
		_Mask("Mask", 2D) = "white" {}
		_MaskScale("Mask缩放", Range(0,10)) = 1
	...
	SubShader{
		...
		Pass {
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
				...
				float3 diffColor = fixed3(_Diffuse_var.rgb*_DiffusePower*_Diffuse_var.a);
				...
				// float3 emissiveColor = fixed3(1,1,1);
				// #if _IS_CUSTOM
					float3 emissiveColor = lerp(emissive, _DissolveColor*_DissolveColorPW*_Diffuse_var.a,percentFactor);
				// #else
				// 	emissiveColor = emissive;
				// #endif
				float3 finalColor = emissiveColor*percent*_Mask_var.r;
				#if _HEIGHTGRADIENT_ON
					finalColor.rgb *= saturate((i.posWS.y - _Height )* ( 1.0 - step( i.posWS.y , _Height ) ) /_HeightGradient);
				#endif
				#if _COLOUR_ON
					// #ifndef _IS_CUSTOM
					// 	finalColor.rgb = RGBToHSV(finalColor.rgb);
					// 	finalColor.rgb = lerp(fixed3(0,0,0),HSVToRGB(fixed3((finalColor.r + _Hue)%360,finalColor.g * _Saturation,finalColor.b)),_Contrast);
					// 	finalColor.rgb *= lerp(_SaturLeftColor.rgb,_SaturRightColor.rgb,smoothstep(_SaturLeftColorWeights,_SaturRightColorWeights,_Diffuse_var.r*_Diffuse_var.a));
					// #else
						if(percentFactor==0)
						{
							finalColor.rgb = RGBToHSV(finalColor.rgb);
				...
				// 		half tmpX = i.uv0.x;
				// 	#endif
				// 	fixed3 gradientCol = lerp(_LeftColor.rgb,_RightColor.rgb,smoothstep(_LeftWeights+_Gradient,_RightWeights+_Gradient,tmpX));
				// 	finalColor.rgb *= gradientCol;
				// #endif
				return fixed4(i.vertexColor.rgb*i.vertexColor.a*finalColor*_DiffuseColor.rgb*_DiffuseColor.a,1);
			}
			ENDCG
		}
	}
